...
// Re-export all the chart components from recharts
export const {
  ResponsiveContainer,
  LineChart,
  Line,
...
const Chart = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => {
  return (
    <div
      className={cn("dark:bg-neutral-950 bg-white rounded-md border border-border", className)}
      ref={ref}
      {...props}
    />
  )
...
const ChartContainer = React.forwardRef<
  ...
  React.ComponentProps<"div"> & {
    /** Chart lines/bars etc. keyed by dataKey */
    config?: ChartConfig
          ...
          <div className="grid gap-1.5">
            ...
            >
              <div
                ...
                style={
                  {
                    "--color-bg": color || payload[0]?.payload?.fill || payload[0]?.color,
                    "--color-border": color || payload[0]?.payload?.fill || payload[0]?.color,
                  } as React.CSSProperties
                }
              />
              {tooltipLabel}
...
// Some older components import `ChartLegendItem` from this file.
// It behaves the same as `ChartLegendContent`, so we simply re-export
// an alias to avoid breaking changes.
const ChartLegendItem = ChartLegendContent
